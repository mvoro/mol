...
export function CopyLinkButton({ value }) {
  ...
  return <button type="button" className="pill copy-link-button" data-state={state} data-instant={keyboard || undefined} onClick={copy} aria-label={state === 'copied' ? 'Ссылка скопирована' : state === 'error' ? 'Не удалось скопировать. Повторить' : 'Скопировать ссылку'}>
    <span className="copy-link-layer copy-link-idle" aria-hidden="true"><Copy size={16}/><span>{state === 'error' ? 'Повторить' : 'Скопировать ссылку'}</span></span>
    <span className="copy-link-layer copy-link-done" aria-hidden="true"><Check size={16} weight="regular"/><span>Скопировано</span></span>
...
export function ShareLinkActions({ url, title }) {
  ...
  return <div className="share-link-actions">
    <div className="share-link-targets" aria-label="Поделиться ссылкой">
      <a href={`mailto:?subject=${encodedTitle}&body=${encodeURIComponent(`${title}\n${url}`)}`} aria-label="Отправить по почте" title="Почта"><EnvelopeSimple size={20}/></a>
      ...
      <a href={`https://t.me/share/url?url=${encodedUrl}&text=${encodedTitle}`} target="_blank" rel="noopener noreferrer" aria-label="Поделиться в Telegram" title="Telegram"><PaperPlaneTilt size={20}/></a>
    </div>
