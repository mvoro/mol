...
export function ProjectColorPicker({ value, onChange }) {
  ...
  return <>
    ...
    {open && <Popover id={id} anchorRef={trigger} portalTarget={portalTarget} topLayer label="Цвет проекта" className="project-color-popover" onClose={() => setOpen(false)}>
      ...
      <MenuItem icon={<span className="project-custom-color-dot"/>} aria-expanded={custom} aria-controls={`${id}-custom`} trailing={<ChevronDown size={14} className={custom ? 'is-expanded' : ''} strokeWidth={1.75} aria-hidden="true"/>} onClick={() => {
      ...
      {custom && <div id={`${id}-custom`} className="project-custom-color-controls" onKeyDown={event => {
        ...
        <button type="button" className="project-color-apply" aria-label="Применить свой цвет" disabled={!normalized} onClick={() => choose(normalized)}><Check size={18} strokeWidth={1.75} aria-hidden="true"/></button>
      </div>}
