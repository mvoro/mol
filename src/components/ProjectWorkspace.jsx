...
export function ProjectActions({ project, onAction }) {
  ...
  return <>
    <button ref={anchor} type="button" className="project-action-button" aria-label={`Действия с проектом «${project.name}»`} aria-haspopup="menu" aria-expanded={open} onClick={() => setOpen(value => !value)}><Ellipsis size={20} strokeWidth={1.75} aria-hidden="true"/></button>
    {open && <Popover anchorRef={anchor} label={`Действия с проектом «${project.name}»`} onClose={() => setOpen(false)}>
      <MenuItem icon={<Pencil size={18} strokeWidth={1.75} aria-hidden="true"/>} onClick={() => choose('rename')}>Переименовать</MenuItem>
      <MenuItem icon={<Settings size={18} strokeWidth={1.75} aria-hidden="true"/>} onClick={() => choose('settings')}>Настройки проекта</MenuItem>
      <MenuItem icon={<Share2 size={18} strokeWidth={1.75} aria-hidden="true"/>} onClick={() => choose('share')}>Поделиться</MenuItem>
      <div className="ds-menu-separator" role="separator"/>
      <MenuItem icon={<Trash2 size={18} strokeWidth={1.75} aria-hidden="true"/>} danger onClick={() => choose('delete')}>Удалить проект</MenuItem>
    </Popover>}
  </>;
}
  ...
  return <div className="sidebar-project-group">
    <div className={`project-sidebar-row${active ? ' is-active' : ''}`}>
      ...
      <div className="project-row-actions">
        <button type="button" className="project-action-button" aria-label={`Новый чат в проекте «${project.name}»`} onClick={() => onNewChat(project.id)}><Plus size={16} strokeWidth={1.75} aria-hidden="true"/></button>
...
function ProjectConversation({ chat, onOpen, onAction }) {
  ...
  return <div className="project-conversation" data-menu-open={open || undefined}>
    <button className="project-conversation-main" title={chat.title} onClick={() => onOpen(chat)}><span><strong>{chat.pinned && <Pin className="project-chat-pin" size={13} fill="currentColor" aria-label="Закреплён" strokeWidth={1.75} role="img"/>}{chat.title}</strong><span>{chat.messages?.filter(message => message.text).at(-1)?.text || 'Начните разговор'}</span></span><time>{chat.updatedAt ? new Intl.DateTimeFormat('ru', {day:'numeric',month:'short'}).format(chat.updatedAt) : ''}</time></button>
    <button ref={anchor} type="button" className="project-action-button project-conversation-more" aria-label={`Действия с чатом «${chat.title}»`} aria-haspopup="menu" aria-expanded={open} onClick={() => setOpen(value => !value)}><Ellipsis size={20} strokeWidth={1.75} aria-hidden="true"/></button>
  ...
  return createPortal(<div className="mc-project-dialog-overlay" data-closing={closing || undefined} onPointerDown={event => { if(event.target === event.currentTarget) requestClose(); }}>
    <section ref={dialogRef} className="mc-project-dialog project-settings-dialog" role="dialog" aria-modal="true" aria-labelledby={`${id}-title`} tabIndex={-1} data-closing={closing || undefined}>
      ...
      {action === 'delete' ? <><p>Удалить проект «{project.name}»? Его чаты сохранятся в общей истории без проекта.</p><footer><button className="pill" onClick={requestClose}>Отмена</button><button className="pill project-danger-solid" onClick={() => { onDelete(project.id); requestClose(); }}>Удалить проект</button></footer></> : action === 'share' ? <>
        <p>Ссылка на проект «{project.name}»</p><div className="project-share-link"><Link size={18} strokeWidth={1.75} aria-hidden="true"/><TextInput aria-label="Ссылка на проект" value={shareUrl} readOnly onFocus={event => event.target.select()}/></div>
        ...
        <footer>{action === 'settings' && <button type="button" className="pill project-danger" onClick={() => onAction('delete', project)}><Trash2 size={15} strokeWidth={1.75} aria-hidden="true"/>Удалить</button>}<button type="button" className="pill" onClick={requestClose}>Отмена</button><button type="submit" className="pill mc-project-create" disabled={Boolean(error) || closing}>Сохранить</button></footer>
      </form>}
    </section>
  </div>, document.body);
}
